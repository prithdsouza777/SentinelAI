"""LLM service with Anthropic API and MockLLM safety net.

Priority: Anthropic direct API > MockLLM
...
class AnthropicLLM:
    """Direct Anthropic API — primary LLM provider."""
    ...
    async def invoke(self, prompt: str, context: dict | None = None) -> dict:
        ...
        try:
            ...
            logger.warning("Anthropic API call failed: %s. Falling back to mock.", e)
...
class BedrockService:
    """Auto-selects: Anthropic API > MockLLM."""
    ...
    def _get_llm(self):
        ...
        # 2. Fallback to mock
        logger.info("Using MockLLM — set ANTHROPIC_API_KEY for real AI")
    ...
    async def invoke_with_system(self, system_prompt: str, user_prompt: str, max_tokens: int = 512) -> dict:
        ...
        if isinstance(llm, AnthropicLLM):
            try:
                ...
                raise  # Let calling agent catch and fall back to threshold
            except Exception as e:
